...
class DataMapper {
  ...
  _ensureIncludeFields (request) {
    ...
    if (includeOption && includeOption.length && options.fieldsOnly && options.fieldsOnly.length) {
  ...
  include (context) {
    ...
    const includeFields = _.reduce(
      primaryModelDescriptors,
      (result, desc, key) => {
        if (desc.link && desc.isArray) { // is hasMany link
          ...
          if (index !== -1) { // hasMany link is already in "include" option
            const options = result[index][1]
            if (options && options.fieldsOnly) { // merge 'fieldsOnly' option
              options.fieldsOnly = _.uniq(options.fieldsOnly, defaultOptions.fieldsOnly)
            }
          } else {
    ...
    // const hasManyFields = [['users', {fieldsOnly: ['id', 'group']}]]

    if (!primaryRecords || !includeFields || !includeFields.length) return context
    ...
    return Promise.all(includeFields.map((linkFieldDescriptor) => {
      ...
      if (fieldDescriptor.isArray) { // hasMany
        const inverseFields = _.reduce(
          this.recordTypes[relationType], // relation model
          (acc, descr, key) => {
            if (descr.link && (descr.link === primaryType)) acc.push(key) // is inverse link
            return acc
          },
          []
        )

        // checking that inverse belongsTo exists is already done
        const foreignKeyField = inverseFields[0] // for now only one inverse relation TODO
  ...
  _getInverseLink (primaryType, isArrayLinkName) {
    ...
    const inverseKey = _.findKey(relationDescriptors, { inverse: isArrayLinkName })

    if (inverseKey) return inverseKey

    if (_.filter(relationDescriptors, { link: primaryType }).length > 1) {
      throw new Error(`_getInverseLink: ambiguous inverse relation in '${relationType}' w/ link='${primaryType}'. use "inverse" keyword to disambiguate`)
    }

    if (_.filter(primaryDescriptors, { link: relationType }).length > 1) {
      throw new Error(`_getInverseLink: ambiguous isArray relations in '${primaryType}' w/ link='${relationType}'. something is wrong in the schema`)
    }

    return _.findKey(relationDescriptors, { link: primaryType })
  }
  ...
  _validateIncludeOption (type, includeOption) {
    ...
    includeOption.forEach((linkFieldDescriptor) => {
      ...
      const relationModelDescriptors = this.recordTypes[relationType]
      if (!relationModelDescriptors) {
        throw new TypeError(`include: '${linkField}' link of '${type}' points` +
        ` to undescribed '${relationType}'`)
      }

      // hasMany relations
      if (fieldDescriptor.isArray) {
        const inverseBelongsTo = _.filter(
          relationModelDescriptors,
          (descr, key) => { // is inverse belongsTo
            if (descr.link && descr.isArray) {
              throw new TypeError(`include: '${type}' hasMany '${linkField}' pointing` +
              ` to '${relationType}' which has inverse '${key}' isArray link`)
            }
            return descr.link && (descr.link === type)
          }
        )

        // _.filter() always returns an array
        if (!inverseBelongsTo.length) {
          throw new TypeError(`include: '${type}' hasMany '${linkField}' pointing` +
          ` to '${relationType}' which has no inverse belongsTo relation`)
        }

        if (inverseBelongsTo.length > 1) {
          throw new TypeError(`Is not implemented. include: '${type}' hasMany` +
          ` '${linkField}' points to '${relationType}' which has more than one inverse relation`)
        }
      }
