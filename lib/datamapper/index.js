...
class DataMapper {
  ...
  update (context) {
    ...
    .then((transformedUpdates) => {
      const links = getLinks(this.recordTypes[request.type])
      if (!Object.keys(links).length) return transformedUpdates // nothing to do

      return Promise.all(
        transformedUpdates.map((record) => this._ensureReferentialIntegrity(record, links))
      )
    })
