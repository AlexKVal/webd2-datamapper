...
class DataMapper {
  ...
  create (context) {
    ...
    .then((newTransaction) => {
      context.transaction = transaction = newTransaction
    })
    ...
    .catch((error) => {
      ...
      .then(() => {
        if (transaction) return transaction.endTransaction(error)
      })
      .then(() => { throw error })
    })
  ...
  delete (context) {
    ...
    return this.adapter.connect()
    .then(() => this.adapter.find(request.type, {ids}))
    .then((records) => {
    ...
    .then((records) => transaction.delete(request.type, request.ids))

    // here goes referential integrity
    ...
    .catch((error) => {
      ...
      return transaction.endTransaction(error)
      .then(() => { throw error })
    })
