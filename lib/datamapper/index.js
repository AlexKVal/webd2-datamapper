...
class DataMapper {
  ...
  create (context) {
    ...
    .then((records) => this._ensureRIofAllRecords(request.type, records))
  ...
  update (context) {
    ...
    if (!updates || !updates.length) {
      return Promise.reject(new BadRequestError('UpdateRecordsInvalid'))
    }

    // ids should be present and be more than 0
    if (!_.every(updates, 'id')) {
      return Promise.reject(new BadRequestError('UpdateRecordMissingID'))
    }
    ...
    .then(() => { // should we apply transform function
      const transformers = this.transforms[primaryType]
      const transformer = transformers && transformers.input && transformers.input.update

      if (!transformer) return updates

      const ids = updates.map((u) => u.id)

      return transaction.find(primaryType, {ids})

      .then((records) => Promise.all(updates.map((update) => {
        const record = _.find(records, {id: update.id})
        // `update` transformer receives `records` - previous state - and `updates`
        return transformer(context, record, update)
      })))
    })

    .then((transformedUpdates) => this._ensureRIofAllRecords(request.type, transformedUpdates))

    // apply updates
    .then((validatedUpdates) => transaction.update(primaryType, validatedUpdates))
  ...
  include (context) {
    ...
    const includeOption = request.include
    const fieldsOnly = request.options && request.options.fieldsOnly

    if (!primaryRecords || !primaryRecords.length) return context

    // includeOption is validated in 'find()' at this moment
    const includeFields = this._mergeIncludeWithArrayLinks(primaryType, includeOption, fieldsOnly)
   ...
   * ]
   */
  _mergeIncludeWithArrayLinks (primaryType, includeOption, fieldsOnly) {
    if (fieldsOnly && !_.includes(fieldsOnly, 'id')) {
      if (includeOption && includeOption.length) {
        throw new BadRequestError('wrong query: `include` option with `id` filtered out by `fieldsOnly`')
      }
    ...
    if (fieldsOnly) { // fieldsOnly should not filter out requested links
      includeOptionFieldsNameArray.forEach((includeName) => {
        if (!_.includes(fieldsOnly, includeName)) {
          throw new BadRequestError('wrong query: `include` option is in conflict with `fieldsOnly`')
        }
      })
    }
    ...
    return _.reduce(
      ...
      (result, desc, key) => {
        ...
        } else if (_.includes(fieldsOnly, key)) { // fieldsOnly can request array links as well
          result.push([key, defaultOptions])
        }
  ...
  _ensureRIofAllRecords (type, records) {
    const links = getLinks(this.recordTypes[type])
    if (!Object.keys(links).length) return records // nothing to do
