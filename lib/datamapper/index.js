  ...
  include (context) {
    ...
    return Promise.all(includeOption.map((linkFieldDescriptor) => {
      ...
      if (fieldDescriptor.isArray) { // hasMany
        // check inverse belongsTo exist
        // const relationModelDescriptors = this.recordTypes[relationType]
        // relationModelDescriptors.filter()

        // fKeyField

        // fetch foreign records by foreign primaryKey IN ( primaryRecords[fKeyField] )
        // 'WHERE primaryKey IN (foreignIDs)'
        const primaryIDs = primaryRecords.map((record) => record.id)
        const options = {match: {group: primaryIDs}}

        return this.adapter.find(relationType, options)
        .then((foreignRecords) => {
          // get foreignIDs
          const foreignIDs = foreignRecords.map((record) => record[fKeyField])
          if (!foreignIDs || !foreignIDs.length) return // nothing to do
        })
        ...
        // embed hasMany ids into primaryRecords
        // filter only ids for each particular record
        // and merge ids into the primary record
      } else { // belongsTo
