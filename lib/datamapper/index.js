  ...
  include (context) {
    ...
    return Promise.all(includeOption.map((linkFieldDescriptor) => {
      ...
      if (fieldDescriptor.isArray) {
        ...
        // belongsTo
        const ids = primaryRecords.map((record) => record[linkField])
        ...
        .then((records) => { include[relationType] = records })
      }
    }))
    .then(() => { response.include = include })

    // here response.include is filled with data
    .then(() => context)
  }
