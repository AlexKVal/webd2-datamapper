...
test('dataMapper._mergeIncludeWithArrayLinks(primaryType, includeOption, fieldsOnly)', (t) => {
  ...
  t.deepEqual(
    mapper._mergeIncludeWithArrayLinks('person', undefined, undefined),
    [
      ['ownPets', {fieldsOnly: ['id', 'owner']}],
      ['accounts', {fieldsOnly: ['id', 'user']}]
    ],
    'includeOption & fieldsOnly arguments are optional'
  )

  t.deepEqual(
    mapper._mergeIncludeWithArrayLinks('person', undefined, ['name']),
    [],
    "if 'id' field is filtered out by fieldsOnly and there is no includeOption"
  )

  t.deepEqual(
    mapper._mergeIncludeWithArrayLinks('person', undefined, ['name', 'ownPets', 'accounts']),
    [],
  ...
  t.deepEqual(
    mapper._mergeIncludeWithArrayLinks('person', undefined, ['id', 'accounts']),
    [
      ['accounts', {fieldsOnly: ['id', 'user']}]
    ],
...
test('dataMapper.update()', (t) => {
  ...
  adapter.find = (type, options) => {
    ...
    //     options,
    //     {ids: [200]},
    //     'it checks ref.integrity'
    //   )
    //   return Promise.resolve([{id: 23, name: 'Admins'}])
    // }
  }
  adapter.update = (type, records) => {
    t.equal(type, 'user')
