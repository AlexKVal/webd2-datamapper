...
test.skip('dataMapper.include()', (t) => {
  const recordTypes = {
    user: {
      name: { type: String },
  ...
  const adapter = new DataMapper(recordTypes)
...
test('dataMapper._validateIncludeOption()', (t) => {
  ...
  t.throws(
    () => adapter._validateIncludeOption('user', 'include option is not an array'),
    /"include" option should be an array/
  )

  t.throws(
    () => adapter._validateIncludeOption('user', [ 'notAnArray' ]),
    /"include" 'notAnArray' field descriptor should be an array/
  )

  t.throws(
    () => adapter._validateIncludeOption('user', [ ['undescribedField'] ]),
    /include: there is no 'undescribedField' field in 'user' type/
  )

  t.throws(
    () => adapter._validateIncludeOption('user', [ ['notALinkField'] ]),
    /include: 'notALinkField' field is not a link/
  )

  t.throws(
    () => adapter._validateIncludeOption('user', [ ['group', 'options'] ]),
