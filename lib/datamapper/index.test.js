...
test('dataMapper.find()', (t) => {
  ...
  dataMapper._ensureFieldsOnlyAndInclude = () => t.pass('it calls _ensureFieldsOnlyAndInclude()')
...
test('dataMapper._ensureFieldsOnlyAndInclude()', (t) => {
  const recordTypes = {
    user: {
      name: { type: String },
      group: { link: 'group' }, // belongsTo
  ...
  t.deepEqual(
    mapper._ensureFieldsOnlyAndInclude({
      type: 'user',
...
test('dataMapper._ensureFieldsOnlyAndInclude() uses `include` option validation', (t) => {
  t.plan(1)
