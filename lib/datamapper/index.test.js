...
test.skip('dataMapper.include() primary model has no hasMany links', (t) => {
...
test.skip('dataMapper.include() w/o "include" option but w/ hasMany in primary model', (t) => {
  // t.plan(3)
...
test('dataMapper._validateIncludeOption()', (t) => {
  const recordTypes = {
    user: {
      ...
      group: { link: 'group' }, // belongsTo
      undescribedDivision: { link: 'undescribedModel' } // broken belongsTo
    ...
    account: {
      customers: { link: 'user', isArray: true } // hasMany w/o inverse belongsTo
    },

    one: {
      twos: { link: 'two', isArray: true }
    },
    two: {
      ones: { link: 'one', isArray: true }
    }
  }
  ...
  t.throws(
    () => mapper._validateIncludeOption('user', [ ['undescribedDivision'] ]),
    /include: 'undescribedDivision' link of 'user' points to undescribed 'undescribedModel'/
  )

  t.throws(
    () => mapper._validateIncludeOption('account', [ ['customers'] ]),
    /include: 'account' hasMany 'customers' pointing to 'user' which has no inverse belongsTo relation/,
    'wrong hasMany relation'
  )
  ...
  t.throws(
    () => mapper._validateIncludeOption('two', [ ['ones'] ]),
    /include: 'two' hasMany 'ones' pointing to 'one' which has inverse 'twos' isArray link/,
    'right hasMany relation'
  )
...
test('dataMapper._getInverseLink(primaryType, isArrayLinkName)', (t) => {
  ...
  t.throws(
    () => new DataMapper({
      ...
      pet: { // ambiguous inverse
        owner: { link: 'person' },
        groomer: { link: 'person' }
      }
    })._getInverseLink('person', 'ownPets'),
    /_getInverseLink: ambiguous inverse relation in 'pet' w\/ link='person'/
  )

  t.throws(
    () => new DataMapper({
      person: { // wrong schema: two hasMany
        ownPets: { link: 'pet', isArray: true },
        groomingPets: { link: 'pet', isArray: true }
      },
      pet: { // and only one inverse belongsTo
        owner: { link: 'person' }
      }
    })._getInverseLink('person', 'ownPets'),
    /_getInverseLink: ambiguous isArray relations in 'person' w\/ link='pet'/
  )
