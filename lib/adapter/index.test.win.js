...
test('I&T adapter.create()', (t) => {
  ...
  prepareTestTables(t, {groups: [], users: []}) // we need empty tables
  ...
  .then((users) => t.deepEqual(
    ...
    [
      ...
      {id: 3, name: 'Partial', deleted: false, group: 0},
      {id: 4, name: 'just a name', deleted: false, group: 0}
    ],
...
test('I&T adapter.update()', (t) => {
  ...
  .then((count) => t.equal(count, 2, 'number of rows has been updated'))
  .then(() => adapter.find('group'))
  .then((groups) => t.deepEqual(
    groups,
    [
  ...
  .then((count) => t.equal(count, 5, 'number of rows has been updated'))
  .then(() => adapter.find('user'))
  .then((users) => t.deepEqual(
    users,
    [
