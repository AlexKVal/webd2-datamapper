...
class Adapter {
  ...
  /**
   * Returns array of clauses for public data fields only.
   * No `id`. No relations. No private fields.
   *
   * ['PersName as name', 'hide', 'enabled']
   */
  _dataColumnsPart (type) {
    const allDescriptors = this.recordTypes[type]

    return Object.keys(allDescriptors).reduce((memo, key) => {
      const field = allDescriptors[key]
      if (key !== '$id' && key !== '$table' && !field.link && !field.private) {
        memo.push(key)
      }
      return memo
    }, [])
