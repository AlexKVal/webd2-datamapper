...
const debug = require('debug')('webd2-api:adapter')
...
class Adapter {
  ...
  create (type, records) {
    ...
    if (records) {
      if (!Array.isArray(records)) throw new TypeError('records should be of Array type')
      if (!records.length) return Promise.resolve([]) // no-op
    }

    return Promise.all(records.map((record) =>
      this.query(this._buildCreateQuery(type, record))
      .then(() => this.query(this._buildGetNewIdQuery(type)))
      .then((rows) => Number(rows[0].newId))
    ))
    .then((newIds) => this.find(type, {ids: newIds})) // return newly created records
  }
  ...
  outputRecord (type, record) {
    ...
    for (let field in record) {
      ...
      const fieldType = ((isId || descriptor.link) && Number) || descriptor.type
  ...
  _buildGetNewIdQuery (type) {
    ...
    return `SELECT MAX("${idFieldName}") FROM "${tableName}"`
  }
    ...
    for (let fieldName in record) {
      ...
      if (fieldName === 'id') {
        ...
        if (fieldDescr.link && fieldDescr.isArray) {
          throw new Error('inserting of hasMany is not implemented')
        }
