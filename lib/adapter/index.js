...
class Adapter {
   ...
   */
  delete (type, ids) {
    if (ids && !ids.length) return Promise.resolve(0) // no-op
    ...
    return this.query(this._buildCountRowsQuery(type, ids))
    .then((rows) => { numberOfRowsToBeDeleted = Number(rows[0].count) })

    .then(() => this.query(this._buildDeleteQuery(type, ids)))
  ...
  _buildCountRowsQuery (type, ids) {
    //
  }

  _buildDeleteQuery (type, ids) {
    //
  }
}
