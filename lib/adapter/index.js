...
class Adapter {
  ...
  connect () {
    if (this.dbClient) throw new Error('there is opened connection already')
   ...
   * completed successfully, or rejects the promise if it failed.
   *
   * If an error is passed, roll back the transaction.
   */
  endTransaction (errorArg) {
    return new Promise((resolve, reject) =>
      this.dbClient.query(errorArg ? 'ROLLBACK WORK' : 'COMMIT WORK', (error) => {
        if (error || errorArg) reject(error || errorArg)
        resolve()
      })
    )
  }
