...
class Adapter {
  ...
  _wherePart (type, options) {
    ...
    for (let matchField in options.match) {
      ...
      if (~describedFields.indexOf(matchField)) { // even private
        const fieldType = descriptors[matchField].type
    ...
    for (let rangeField in options.range) {
      ...
      const fieldType = isIdField ? Number : descriptors[rangeField].type
