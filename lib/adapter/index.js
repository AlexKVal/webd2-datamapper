...
class Adapter {
   ...
   */
  find (type, options) {
    if (!this.recordTypes[type]) throw new TypeError(`there is no "${type}" described`)

    options = options || {}

    // Handle no-op.
    if (options.ids && !options.ids.length) return Promise.resolve([])
  ...
  _wherePart (type, options) {
    ...
    if (options.ids) {
      if (!_.every(options.ids, Number)) throw new TypeError('options.ids can only be of Number type')
      clauses.push(`${(descriptors.$id || 'id')} IN (${options.ids.join(', ')})`)
    }

    if (options.match) {
      const describedFields = Object.keys(descriptors)

      _.transform(options.match, (result, whereValue, whereField) => {
        if (whereValue === undefined) {
          throw new Error(`match: value of the '${whereField}' field is undefined`)
        }

        if (~describedFields.indexOf(whereField)) { // even private
          const fieldType = descriptors[whereField].type
          const fieldName = descriptors[whereField].$fname || whereField

          if (Array.isArray(whereValue)) {
            result.push(`${fieldName} IN (...)`)
          } else {
            const fieldValue = mapValue(fieldType, whereValue)
    ...
    // return Object.keys(options.match).reduce((memo, fieldName) => {
    //   const value = whereOptions[fieldName]
    //
    //   if (value === undefined) {
    //     throw new TypeError(`_wherePart(): value of '${fieldName}' key is undefined`)
    //   }
    //
    //   if (
    //     key[0] !== '$' && // exclude meta
  ...
  _buildSelectQuery (type, options) {
    ...
    const selectColumns = 'SELECT ' +
      this._selectFieldsPart(type, options.fieldsOnly) +
      ` FROM ${tableName}`

    const where = this._wherePart(type, options)

    const order = ''

    const slice = ''

    return `${selectColumns} ${where} ${order} ${slice}`.trim()
  }
}
...
Adapter.mapValue = mapValue // for testing
