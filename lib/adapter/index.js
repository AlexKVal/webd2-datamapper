...
class Adapter {
  ...
  _wherePart (type, options) {
    ...
    if (options.match) {
      ...
      return _.transform(options.match, (result, whereValue, whereField) => {
        if (whereValue === undefined) {
          throw new Error(`wherePart: value of the '${whereField}' field is undefined`)
        }

        if (~describedFields.indexOf(whereField)) { // even private
          const fieldType = descriptors[whereField].type
          result.push(`${whereField}=${this._mapValue(fieldType, whereValue)}`)
        }
      }, clauses)
    }
