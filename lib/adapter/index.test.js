...
test('Adapter', (t) => {
  ...
  const instance = new Adapter({options: {dsn: 'DB'}, recordTypes})
...
test('adapter._dataColumnsPart()', (t) => {
  ...
  let recordTypes = {
    aType: {
      ...
      name: { type: String },
      hide: { type: Boolean },
      private: { type: Number, private: true },
      group: { link: 'group' }
    }
  }

  t.deepEqual(
    new Adapter({options, recordTypes})._dataColumnsPart('aType'),
    ['name', 'hide'],
    'returns only data fields'
  )
