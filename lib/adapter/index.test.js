...
test('adapter._buildCountRowsQuery(type, ids)', (t) => {
  let recordTypes = {}

  recordTypes.user = {/* empty */}
  t.equal(
    new Adapter({recordTypes})._buildCountRowsQuery('user'),
    'SELECT COUNT(*) as "count" FROM "user"',
    'with empty `ids` it counts all rows'
  )
  ...
  t.equal(
    new Adapter({recordTypes})._buildCountRowsQuery('user'),
    'SELECT COUNT(*) as "count" FROM "sPersonal"',
    'it uses `$table` for table name'
  )
  ...
  t.equal(
    new Adapter({recordTypes})._buildCountRowsQuery('user', [100, 101, 123]),
    'SELECT COUNT(*) as "count" FROM "sPersonal" WHERE "UserID" IN (100, 101, 123)',
    'it counts only rows with provided `ids` and uses provided `$id`'
  )
