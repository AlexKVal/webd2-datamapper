...
test('adapter._buildCreateQuery(type, record)', (t) => {
  ...
  t.throws(
    () => adapter._buildCreateQuery('user', record),
    /inserting of hasMany is not implemented/,
    'it not the Adapter`s concern'
  )
...
test('adapter.update(type, records)', (t) => {
  ...
  adapter._buildDoesRowExist = (type, id) => {
    t.pass('it calls this._buildDoesRowExist() for every record')
    return `SELECT "id" FROM "aType" WHERE "id"=${id}`
  }
  ...
  .then(() => adapter.update('aType', [{id: 1, name: 'new name'}, {id: 2, name: 'next'}]))
  .then((res) => t.equal(res, 2, 'has been updated 2 rows'))
...
// _buildUpdateQuery
// .then(() => adapter.update('aType', [{name: 'new name'}])) // no id
// .catch((e) => t.equal(e.message, 'update: the record has no "id"'))
